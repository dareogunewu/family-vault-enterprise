-- Session Management Enhancement for 30-minute timeout
...
-- Function to extend session activity
...
-- Set up automated cleanup job (run every 5 minutes)
-- Note: This requires pg_cron extension which may not be available on all Supabase plans
-- SELECT cron.schedule('cleanup-expired-sessions', '*/5 * * * *', 'SELECT public.cleanup_expired_sessions();');

-- Alternative: Create a function that can be called from your application periodically
